...
dp=Dispatcher()
DB='data_v2.json' # Изменил имя, чтобы у всех очистились карты
CD=18000 # 5 часов
...
def save(d):
    with open(DB,'w',encoding='utf-8') as f: json.dump(d,f,ensure_ascii=False)
...
def get_kb():
    ...
    kb.button(text="Очистить коллекцию")
    kb.adjust(2, 1)
    return kb.as_markup(resize_keyboard=True)

@dp.message(Command("start"))
async def s(m: types.Message):
    await m.answer("🦶 Бот обновлен! Инвентари очищены, кд 5 часов.", reply_markup=get_kb())

@dp.message(F.text == "Очистить коллекцию")
async def clear_inv(m: types.Message):
    d = load(); u = str(m.from_user.id)
    if u in d:
        del d[u]; save(d)
        await m.answer("♻️ Твоя коллекция и таймер полностью очищены!")
    else:
        await m.answer("У тебя и так пустой инвентарь.")

@dp.message(F.text.lower() == "пятка")
async def g(m: types.Message):
    d=load();u=str(m.from_user.id);n=time.time()
    if u in d and n-d[u].get('t',0)<CD:
        rem=int(CD-(n-d[u]['t']))
        return await m.answer(f"⏳ Жди {rem//3600}ч. {(rem%3600)//60}м.")
    if u not in d: d[u]={'i':[],'t':0}
    
    av=[(r,i) for r,items in DATA.items() for i in items if i not in d[u]['i']]
    if not av: return await m.answer("🏆 Ты собрал всё! Можешь очистить коллекцию, чтобы начать заново.")
    
    rk=random.choices(list(DATA.keys()),weights=CH,k=1)
    ps=[x for x in DATA[rk] if x not in d[u]['i']]
    if not ps: rk,name=random.choice(av)
    else: name=random.choice(ps)
    
    d[u]['i'].append(name); d[u]['t']=n; save(d)
...
async def v(m: types.Message):
    d=load();i=d.get(str(m.from_user.id),{}).get('i',[])
    await m.answer("📜 Коллекция:\n"+"\n".join([f"— {x}" for x in i]) if i else "Пусто.", reply_markup=get_kb())
